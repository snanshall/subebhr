                ...
                <table class="table table-striped" cellspacing="0" width="100%">
                    ...
                    foreach ($menu_info as $items) {
                        ...
                        <script type="text/javascript">
                            ...
                            || $items->label == 'report_by_month' || $items->label == 'stock_history' || $items->label == 'assign_stock_report' || $items->label == 'stock_report' || $items->label == 'overtime' || $items->label == 'performance_report' || $items->label == 'database_backup'
                            ...
                            || $items->label == 'email_notification' || $items->label == 'translations' || $items->label == 'system_update' || $items->label == 'private_chat'
                                ){?>
                                $('.<?= $items->menu_id?>').prop('disabled', true);
                                $(".<?= $items->menu_id?>").next().css('display', 'none');
                                $('.view .<?= $items->menu_id?>').prop('disabled', false);
